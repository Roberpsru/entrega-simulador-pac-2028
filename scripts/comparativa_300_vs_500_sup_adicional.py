"""ENTREGABLE — >300  vs  >500 + superficie adicional (las CUATRO categorías).
...
Escenario B = ">500 + superficie adicional": primero se aplica el umbral 500; sobre los
titulares SUPERVIVIENTES (>500) se activan las cuatro palancas del Filtro B:
  · Cultivos externos (viñedo y txakoli, frutales y frutos secos, hortícolas): el
    ...
    la huella de cultivo de los supervivientes >500 (proporciones del >500, total fijo).
  · ABRS sin derecho REACTIVADA AL 100 %: ha_abrs_sin_derecho_reactivada = exceso
    completo Σ max(ABRS − DERECHOS, 0) calculado SOLO sobre los titulares >500 (es
...
Uso:
  python scripts/comparativa_300_vs_500_sup_adicional.py            -> CHECKPOINT (no escribe)
  python scripts/comparativa_300_vs_500_sup_adicional.py --generar  -> genera el Excel
"""
...
OUT = RAIZ / "comparativa_300_vs_500_superficie_adicional.xlsx"
UMBRAL_A = 300
UMBRAL_B = 500
...
def escenario_b(df: pd.DataFrame) -> dict:
    """Umbral 500 + Filtro B (4 palancas) sobre los supervivientes >500."""
    ...
    # supervivientes >500 (proporciones del >500, total invariante). ──
    ...
    for tipo, col in CAT_SIGPAC.items():
        ...
        decl_surv = float(survivors[cols_ok].sum().sum()) if cols_ok else 0.0  # huella >500
    ...
    # ── ABRS sin derecho: exceso completo de los supervivientes >500 (reactivación 100 %) ──
    ...
    n_exact_500 = int((benef["IMP_AYUDA_TOTAL"] == UMBRAL_B).sum())
    return dict(
        ...
        n_survivors=int(len(survivors)), n_exact_500=n_exact_500,
    )
...
def checkpoint(b: dict):
    print("\n" + "=" * 72)
    print(f"CHECKPOINT B — titulares >500 (supervivientes: {b['n_survivors']})")
    print("=" * 72)
    print(f"  Titulares con ayuda exactamente = {UMBRAL_B} €: {b['n_exact_500']} "
          f"(0 ⇒ '>500' estricto equivale a la convención de la app)")
    ...
    mostrar("Externas (cultivos) a incorporar — dato real por titular, base >500",
            pd.DataFrame(rep_ext)[["Categoría", "Araba", "Gipuzkoa", "Bizkaia", "Euskadi"]])

    # ABRS sin derecho exceso por territorio
    abrs = [{"Concepto": "ABRS sin derecho (exceso, base >500)",
             **{t: round(b["exc_por_terr"][t], 2) for t in TERRS},
             "Euskadi": round(b["exceso_total"], 2)}]
    mostrar("ABRS sin derecho a reactivar (100 %) — exceso titular a titular, base >500",
...
def cuadro(sim_a: pd.DataFrame, b: dict, col_a: str, key_b: str) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        ">500 + sup. adicional": [round(float(b["filas"][t][key_b]), 2) for t in ORDEN],
...
def main():
    ...
    NOTA = (
        ...
        f">500: cultivos externos ({_eu(b['total_externas'])} ha, dato real por titular) y "
        f"reactivación del 100 % de la ABRS sin derecho ({_eu(b['exceso_total'])} ha, exceso "
        "titular a titular). El umbral 500 se aplica PRIMERO; las superficies adicionales son "
        "solo las de los supervivientes, no las de los excluidos <500.  "
