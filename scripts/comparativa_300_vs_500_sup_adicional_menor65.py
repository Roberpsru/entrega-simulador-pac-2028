"""ENTREGABLE — >300  vs  >500 + superficie adicional + <65 (las CUATRO palancas).
...
Escenario B = ">500 + superficie adicional + menores de 65": se aplican EN ESTE ORDEN
  1) umbral 500 (ayuda > 500 €),
  2) exclusión de los ≥65 (EDAD = 2024 − año FNACIMIENTO; solo personas FÍSICAS;
     las jurídicas y los titulares sin fecha de nacimiento se mantienen),
  3) Filtro B con las cuatro palancas sobre los supervivientes (>500 y <65):
     · Cultivos externos (viñedo y txakoli, frutales y frutos secos, hortícolas):
       máximo incorporable por categoría sobre la BASE AMPLIA (ayuda > 0), igual que
       comparativa_sup_adicional.py (total ≈6.298,72 ha); reparto territorial real por
       titular con la huella de cultivo de los supervivientes (proporciones, total fijo).
     · ABRS sin derecho REACTIVADA AL 100 %: ha_abrs_sin_derecho_reactivada = exceso
       completo Σ max(ABRS − DERECHOS, 0) calculado SOLO sobre los supervivientes.

Las superficies adicionales son únicamente las de los supervivientes (no las de los
excluidos por umbral ni por edad).
...
Uso:
  python scripts/comparativa_300_vs_500_sup_adicional_menor65.py            -> CHECKPOINT
  python scripts/comparativa_300_vs_500_sup_adicional_menor65.py --generar  -> genera el Excel
"""
...
OUT = RAIZ / "comparativa_300_vs_500_sup_adicional_menor65.xlsx"
UMBRAL_A = 300
UMBRAL_B = 500
EDAD_CORTE = 65
...
def escenario_b(df: pd.DataFrame) -> dict:
    """Umbral 500 → exclusión ≥65 (físicas) → Filtro B (4 palancas) sobre supervivientes."""
    benef = df[df["IMP_AYUDA_TOTAL"] > 0].copy()
    presupuesto = float(benef["IMP_AYUDA_TOTAL"].sum())           # constante (toda la base)

    # 1) umbral 500 (estricto)
    surv_umbral = benef[benef["IMP_AYUDA_TOTAL"] > UMBRAL_B].copy()
    # 2) exclusión ≥65 solo personas FÍSICAS (jurídicas y sin fecha se mantienen)
    es_fis = (surv_umbral["ES_PERSONA_FISICA"].fillna(False)
              if "ES_PERSONA_FISICA" in surv_umbral else pd.Series(True, index=surv_umbral.index))
    mask_65 = (surv_umbral["EDAD"] >= EDAD_CORTE) & es_fis
    survivors = surv_umbral[~mask_65].copy()

    # Recuento de exclusiones (orden: umbral primero, edad después)
    n_excl_umbral = int(len(benef) - len(surv_umbral))
    n_excl_65 = int(mask_65.sum())
    n_jur_65_mantenidas = int(((surv_umbral["EDAD"] >= EDAD_CORTE) & ~es_fis).sum())
    n_sin_edad = int(surv_umbral["EDAD"].isna().sum())
    surv_por_terr = {t: int((survivors["TH_DESC"] == t).sum()) for t in TERRS}
    ...
    # reparto territorial = real por titular con la huella de los supervivientes. ──
    ...
    for tipo, col in CAT_SIGPAC.items():
        ...
        decl_surv = float(survivors[cols_ok].sum().sum()) if cols_ok else 0.0  # huella supervivientes
    ...
    # ── ABRS sin derecho: exceso completo de los supervivientes (reactivación 100 %) ──
    ...
    return dict(
        ...
        n_survivors=int(len(survivors)), surv_por_terr=surv_por_terr,
        n_excl_umbral=n_excl_umbral, n_excl_65=n_excl_65,
        n_jur_65_mantenidas=n_jur_65_mantenidas, n_sin_edad=n_sin_edad,
    )
...
def checkpoint(b: dict):
    print("\n" + "=" * 72)
    print(f"CHECKPOINT B — supervivientes (>500 y <65): {b['n_survivors']}")
    print("=" * 72)
    print(f"  Excluidos por umbral (<= {UMBRAL_B} € de ayuda):       {b['n_excl_umbral']}")
    print(f"  Excluidos por edad (>={EDAD_CORTE}, solo personas físicas): {b['n_excl_65']}")
    print(f"  Personas jurídicas con representante >={EDAD_CORTE} MANTENIDAS: {b['n_jur_65_mantenidas']}")
    print(f"  Titulares sin fecha de nacimiento MANTENIDOS:        {b['n_sin_edad']}")

    surv = [{"Concepto": "Supervivientes (>500 y <65)",
             **{t: b["surv_por_terr"][t] for t in TERRS},
             "Euskadi": sum(b["surv_por_terr"][t] for t in TERRS)}]
    mostrar("Nº de supervivientes por territorio",
            pd.DataFrame(surv)[["Concepto", "Araba", "Gipuzkoa", "Bizkaia", "Euskadi"]])
    ...
    mostrar("Externas (cultivos) a incorporar — dato real por titular, base supervivientes",
            pd.DataFrame(rep_ext)[["Categoría", "Araba", "Gipuzkoa", "Bizkaia", "Euskadi"]])

    abrs = [{"Concepto": "ABRS sin derecho (exceso, base supervivientes)",
             **{t: round(b["exc_por_terr"][t], 2) for t in TERRS},
             "Euskadi": round(b["exceso_total"], 2)}]
    mostrar("ABRS sin derecho a reactivar (100 %) — exceso titular a titular",
...
def cuadro(sim_a: pd.DataFrame, b: dict, col_a: str, key_b: str) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        ">500 + sup. adicional + <65": [round(float(b["filas"][t][key_b]), 2) for t in ORDEN],
...
def main():
    ...
    n_surv_str = f"{b['n_survivors']:,}".replace(",", ".")
    eus = b["filas"]["Euskadi"]
    medio_con_nuevas = (eus["sup_b"] * b["vh"]) / (eus["benef_b"] + total_nuevas) if (eus["benef_b"] + total_nuevas) else 0.0
    NOTA = (
        "NOTA 1 — En B se aplica un DOBLE FILTRO (umbral 500 primero, exclusión de los ≥65 "
        "personas físicas después; jurídicas y sin fecha se mantienen) y, sobre los "
        f"supervivientes ({n_surv_str} explotaciones), las CUATRO palancas del Filtro B: "
        f"cultivos externos ({_eu(b['total_externas'])} ha, dato real por titular) y "
        f"reactivación del 100 % de la ABRS sin derecho ({_eu(b['exceso_total'])} ha, exceso "
        "titular a titular). Las superficies adicionales son solo las de los supervivientes, no "
        "las de los excluidos por umbral ni por edad.  "
    ...
    with pd.ExcelWriter(OUT, engine="openpyxl") as xw:
        hoja = "Comp_300_vs_500_supadic_m65"
        fila = 0
        bloques = [
